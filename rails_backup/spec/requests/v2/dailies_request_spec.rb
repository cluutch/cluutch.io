...
RSpec.describe "V2::Dailies", type: :request do
  ...
  describe "GET /index" do
    ...
    it "returns all avg daily prices" do
      ...
      create(:v2_quote, date: Date.today - 1, price_per_ounce: 5)
      ...
      expect(json[1]["date"]).to eq(Date.today - 1.to_s)
    ...
    it "returns all avg daily prices by specific jurisdiction" do
      ...
      create(:v2_quote, date: Date.today - 1, price_per_ounce: 6, jurisdiction: florida)
      create(:v2_quote, date: Date.today - 1, price_per_ounce: 5)
      create(:v2_quote, date: Date.today - 2, price_per_ounce: 5)
    ...
    describe 'price for specific date' do
      it "returns the exact date if available" do
        ...
        create(:v2_quote, date: Date.today, price_per_ounce: 4)
        create(:v2_quote, date: Date.today - 1, price_per_ounce: 5)
        create(:v2_quote, date: Date.today - 1, price_per_ounce: 7)
        create(:v2_quote, date: Date.today - 7, price_per_ounce: 13)
  
        # when daily prices are queried 
        get "/v2/daily", params: { date: (Date.today - 1).to_s }
  
        # expect two different 
        expect(response).to have_http_status(:success)
        json = JSON.parse(response.body)
        expect(json.length).to eq(1)
        expect(json[0]["date"]).to eq((Date.today - 1).to_s)
      ...
      it "returns the most recent date without going past" do
        ...
        create(:v2_quote, date: Date.today - 1, price_per_ounce: 7)
        create(:v2_quote, date: Date.today - 7, price_per_ounce: 13)
        create(:v2_quote, date: Date.today - 7, price_per_ounce: 15)
        create(:v2_quote, date: Date.today - 9, price_per_ounce: 13)
  
        # when daily prices are queried 
        get "/v2/daily", params: { date: (Date.today - 6).to_s } # in between yesterday and 7 days ago
  
        # expect two different 
        expect(response).to have_http_status(:success)
        json = JSON.parse(response.body)
        expect(json.length).to eq(1)
        expect(json[0]["date"]).to eq((Date.today - 7).to_s)
      ...
      it "returns the most recent date without going past for a specific jurisdiction" do
        ...
        create(:v2_quote, date: Date.today, price_per_ounce: 2)
        create(:v2_quote, date: Date.today, price_per_ounce: 4)
        create(:v2_quote, date: Date.today - 1, price_per_ounce: 5, jurisdiction: florida)
        create(:v2_quote, date: Date.today - 1, price_per_ounce: 7)
        create(:v2_quote, date: Date.today - 7, price_per_ounce: 13)
        create(:v2_quote, date: Date.today - 7, price_per_ounce: 15, jurisdiction: florida)
        create(:v2_quote, date: Date.today - 9, price_per_ounce: 13, jurisdiction: florida)
        create(:v2_quote, date: Date.today - 10, price_per_ounce: 13)
  
        # when daily prices are queried 
        get "/v2/daily", params: { date: (Date.today - 6).to_s, jurisdiction: "Florida" } # in between yesterday and 7 days ago
