...
export const SolanaButton = ({ children, ...props }) => {
  const { connection } = useConnection();
    ...
    return (
        <div className="my-4">
          <button onClick={onClick} type="button" className="btn btn-primary btn-sm">Trigger Solana Update</button><br />
          <span><strong>WARNING: Update 10,000 lamports each!</strong></span><br />
          <span>Funds transferred to an intermediate account. The secret key is outputted to console.</span>
        </div>
    );
};