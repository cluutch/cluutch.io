# open file
# determine strain type, and convert
# determine aroma
# determine feeling
# determine other
# output {}
...
from amnesia_haze import amnesia_haze as strain_json
...
def mutate_strain_json_to_percentages(strain_json, category, options, total):
  ...
  strain_json[category]['best'] = best_match + " (" + str(strain_json[category][best_match]) +  "%)"
...
def get_sanitized_tags(strain_json):
  def filter_irrelevant_tags(tag):
    irrelevant_tags = STRAIN_TYPES + STRAIN_AROMAS + STRAIN_FEELINGS + IGNORE_TAGS
...
def mutate_tags_to_percentages(strain_json, total):
  ...
  strain_json[STRAIN_OTHER_ATTRIBUTES]['best'] = best_match + " (" + str(strain_json[STRAIN_OTHER_ATTRIBUTES][best_match]) +  "%)"
...
def process_strain_json(strain_json):
  ...
  print(json.dumps(strain_json, indent=4))
    ...
    # strain_info[category] = classify_strain_json(strain_json, options)
